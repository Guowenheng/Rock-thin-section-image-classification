...
def init_protonet(opt):
    ...
    model = create_model_swim(num_classes=256,tuning_mode='ssf')
    # weights_path='../dataset/vit_base_patch16_224.pth'
    weights_path = '../dataset/swin_tiny_patch4_window7_224.pth'
    freeze_layers=True
...
def train(opt, tr_dataloader, model, optim, lr_scheduler,ATT,optim_att,att_scheduler, val_dataloader=None):
    '''
    ...
    for epoch in range(opt.epochs):
        ...
        if avg_acc >= best_acc:
            torch.save({'baseline':model.state_dict(),'att':ATT.state_dict()}, best_model_path)

            best_acc = avg_acc
            best_state = {'baseline':model.state_dict(),'att':ATT.state_dict()}
...
def test(opt, test_dataloader, model,ATT):
    '''
...
def eval():
    ...
    ATT = init_MCAM(options)
    model_path = os.path.join(options.experiment_root, 'best_model.pth')
    model_stats=torch.load(model_path)
    print(model_stats['att']['gamma_sar'])
    print(model_stats['att']['gamma_opt'])
    print(model_stats['att']['gamma_att'])
...
def main():
    ...
    ATT = init_MCAM(options)
    optim_att=torch.optim.Adam(params=ATT.parameters(),
                     lr=0.01)
